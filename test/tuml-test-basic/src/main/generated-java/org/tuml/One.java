...
import org.tuml.Many.ManyRuntimePropertyEnum;
...
public class One extends BaseTuml implements TumlNode {
	...
	public void addToMany(Many many) {
		if ( many != null ) {
			many.clearOne();
			many.initialiseProperty(ManyRuntimePropertyEnum.one);
		}
		if ( many != null ) {
			this.many.add(many);
		}
	...
	public void addToMany(TinkerSet<Many> many) {
		if ( !many.isEmpty() ) {
	...
	public void removeFromMany(TinkerSet<Many> many) {
		if ( !many.isEmpty() ) {
	...
	public void setMany(TinkerSet<Many> many) {
		clearMany();
		addToMany(many);
	}
	...
	public enum OneRuntimePropertyEnum implements TumlRuntimeProperty {
		many(false,true,true,"A_<one>_<many>",false,true,false,false,-1,0,false,false,false,false,true),
		name(true,true,false,"basicmodel__org__tuml__One__name",false,false,true,false,1,1,false,false,false,false,true);
		 ...
		 * @param unique 
		 */
		private OneRuntimePropertyEnum(boolean onePrimitive, boolean controllingSide, boolean composite, String label, boolean oneToOne, boolean oneToMany, boolean manyToOne, boolean manyToMany, int upper, int lower, boolean qualified, boolean inverseQualified, boolean ordered, boolean inverseOrdered, boolean unique) {
