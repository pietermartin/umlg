...
import tinker.collectiontest.Dream;
import tinker.collectiontest.Fantasy;
import tinker.collectiontest.Foot;
import tinker.collectiontest.Hand;
import tinker.collectiontest.Nightmare;
import tinker.collectiontest.World;
import tinker.embeddedtest.REASON;
import tinker.hierarchytest.FakeRootFolder;
import tinker.hierarchytest.RealRootFolder;
import tinker.inheritencetest.AbstractSpecies;
import tinker.inheritencetest.Mamal;
import tinker.interfacetest.Being;
import tinker.interfacetest.IMany;
import tinker.interfacetest.Spirit;
import tinker.navigability.NonNavigableMany;
import tinker.navigability.NonNavigableOne;
import tinker.onetoone.OneOne;
import tinker.onetoone.OneTwo;
import tinker.qualifiertest.Many1;
import tinker.qualifiertest.Many2;
import tinker.qualifiertest.Nature;
...
public class God extends BaseTinker implements TinkerNode {
	...
	public Fantasy getFantasyForFantasyQualifierOnName(String fantasyQualifierOnName) {
		Index<Edge> index = GraphDb.getDb().getIndex(getUid() + ":::" + "A_<god>_<fantasy>", Edge.class);
		if ( index==null ) {
			return null;
		} else {
			...
			if ( closeableIterable.iterator().hasNext() ) {
				return new Fantasy(closeableIterable.iterator().next().getVertex(Direction.IN));
			} else {
				return null;
			}
		}
	...
	public Foot getFootForGodFootQualifier(String godFootQualifier) {
		Index<Edge> index = GraphDb.getDb().getIndex(getUid() + ":::" + "A_<god>_<foot>", Edge.class);
		if ( index==null ) {
			return null;
		} else {
			CloseableIterable<Edge> closeableIterable = index.get("godFootQualifier", godFootQualifier==null?"___NULL___":godFootQualifier);
			if ( closeableIterable.iterator().hasNext() ) {
				return new Foot(closeableIterable.iterator().next().getVertex(Direction.IN));
			} else {
				return null;
			}
		}
	...
	public Collection<Nightmare> getMemoryForQualifier1(Set<String> qualifier1) {
		Index<Edge> index = GraphDb.getDb().getIndex(getUid() + ":::" + "A_<god>_<nightmare>_2", Edge.class);
		if ( index==null ) {
			return null;
		} else {
			CloseableIterable<Edge> closeableIterable = index.get("qualifier1", qualifier1==null?"___NULL___":qualifier1);
			if ( closeableIterable.iterator().hasNext() ) {
				return new TinkerSequenceClosableIterableImpl<Nightmare>(closeableIterable);
			} else {
	...
	public Set<Nature> getNatureForQualifier2(Set<String> qualifier2) {
		Index<Edge> index = GraphDb.getDb().getIndex(getUid() + ":::" + "A_<god>_<nature>", Edge.class);
		if ( index==null ) {
			return null;
		} else {
			...
			if ( closeableIterable.iterator().hasNext() ) {
				return new TinkerSetClosableIterableImpl<Nature>(closeableIterable);
			} else {
	...
	public Nightmare getNightmareForQualifier1(String qualifier1) {
		Index<Edge> index = GraphDb.getDb().getIndex(getUid() + ":::" + "A_<god>_<nightmare>", Edge.class);
		if ( index==null ) {
			return null;
		} else {
			CloseableIterable<Edge> closeableIterable = index.get("qualifier1", qualifier1==null?"___NULL___":qualifier1);
			if ( closeableIterable.iterator().hasNext() ) {
				return new Nightmare(closeableIterable.iterator().next().getVertex(Direction.IN));
			} else {
				return null;
			}
		}
	...
	public List<Qualifier> getQualifierXXXForFantasy(Fantasy context) {
	...
	public List<Qualifier> getQualifierXXXForFoot(Foot context) {
		List<Qualifier> result = new ArrayList<Qualifier>();
		result.add(new Qualifier("godFootQualifier", context.getGodFootQualifier(), Multiplicity.ONE_TO_ONE));
		return result;
	}
	
	public List<Qualifier> getQualifierXXXForMemory(Nightmare context) {
		List<Qualifier> result = new ArrayList<Qualifier>();
		result.add(new Qualifier("qualifier1", context.getQualifier1(), Multiplicity.ONE_TO_MANY));
		return result;
	}
	
	public List<Qualifier> getQualifierXXXForNature(Nature context) {
		List<Qualifier> result = new ArrayList<Qualifier>();
	...
	public List<Qualifier> getQualifierXXXForNightmare(Nightmare context) {
		List<Qualifier> result = new ArrayList<Qualifier>();
		result.add(new Qualifier("qualifier1", context.getQualifier1(), Multiplicity.ONE_TO_ONE));
		return result;
	}
	...
	public List<Qualifier> getQualifiers(TumlRuntimeProperty tumlRuntimeProperty, TinkerNode node) {
		...
		if ( runtimeProperty != null && result.isEmpty() ) {
			switch ( runtimeProperty ) {
				case foot:
					result = getQualifierXXXForFoot((Foot)node);
				break;
			
				case nature:
					result = getQualifierXXXForNature((Nature)node);
				break;
			
				case fantasy:
					result = getQualifierXXXForFantasy((Fantasy)node);
				break;
			
				case nightmare:
					result = getQualifierXXXForNightmare((Nightmare)node);
				break;
			
				case memory:
					result = getQualifierXXXForMemory((Nightmare)node);
				break;
	...
	public enum GodRuntimePropertyEnum implements TumlRuntimeProperty {
		...
		animalFarm(false,true,false,"org__tinker__concretetest__God__animalFarm",false,false,false,true,-1,0,false,false,false,false,true),
		pet(false,true,false,"org__tinker__concretetest__God__pet",false,false,true,false,1,1,false,false,false,false,true),
		...
		name(true,true,false,"org__tinker__concretetest__God__name",false,false,true,false,1,1,false,false,false,false,true),
		...
		embeddedInteger(false,true,false,"org__tinker__concretetest__God__embeddedInteger",false,false,false,true,-1,0,false,false,false,false,true),
		...
		reason(false,true,false,"org__tinker__concretetest__God__reason",false,false,true,false,1,1,false,false,false,false,true),
		embeddedString(false,true,false,"org__tinker__concretetest__God__embeddedString",false,false,false,true,-1,0,false,false,false,false,true);
