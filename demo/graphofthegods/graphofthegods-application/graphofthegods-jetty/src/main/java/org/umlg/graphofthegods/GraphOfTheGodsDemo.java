 ...
 * Time: 8:34 PM
 */
public class GraphOfTheGodsDemo {

    public static void main(String[] args) throws Exception {

        URL modelFileURL = Thread.currentThread().getContextClassLoader().getResource("graphofthegods.uml");
        if (modelFileURL == null) {
            throw new IllegalStateException(String.format("Model file %s not found. The model's file name must be on the classpath.", "graphofthegods.uml"));
        }
        ...
        context.setContextPath("/graphofthegods");
        server.setHandler(context);

        //Restlet servlet
        ServletHolder restletServletHolder = new ServletHolder(new ServerServlet());
        restletServletHolder.setName("org.umlg.graphofthegods.GraphofthegodsApplication");
        restletServletHolder.setInitParameter("org.restlet.application", "org.umlg.graphofthegods.GraphofthegodsApplication");
