...
//        get.getBody().addToStatements("json.append(\"[{\\\"data\\\": [{\\\"name\\\": \\\"APP\\\"}]\")");
        ...
        options.getBody().addToStatements("json.append(" + StringUtils.capitalize(model.getName()) + "RuntimePropertyEnum.asJson())");

        annotatedClass.addToImports(UmlgGenerationUtil.UmlgRootPackage.toJavaString() + "." + StringUtils.capitalize(model.getName()) + "." + StringUtils.capitalize(model.getName()) + "RuntimePropertyEnum");
