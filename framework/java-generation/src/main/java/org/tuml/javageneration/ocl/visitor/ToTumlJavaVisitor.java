...
public class ToTumlJavaVisitor extends
	 ...
	 *            my environment
	 */
	protected ToTumlJavaVisitor(Environment<?, Classifier, Operation, Property, EnumerationLiteral, Parameter, State, CallOperationAction, SendSignalAction, Constraint, ?, ?> env) {
		this.env = env;
	 ...
	 */
	public static ToTumlJavaVisitor getInstance(
		...
		return new ToTumlJavaVisitor(env);
	}
	...
	public static ToTumlJavaVisitor getInstance(
			TypedElement<Classifier> element) {
		return new ToTumlJavaVisitor((Environment)Environment.Registry.INSTANCE.getEnvironmentFor(element));
	}
