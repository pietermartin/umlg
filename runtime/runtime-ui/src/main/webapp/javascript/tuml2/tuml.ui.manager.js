...
//                    alert('right arrow');
        ...
        this.refresh = function (tumlUri, pushUrl) {
            ...
            var tumlUriAdjusted = tumlUri;
            //Append umlgRoot to the root url,
            //This is needed because JEE servlet url pattern can not cope with http://localhost:8111/appName
            //It needs something after the appName
            if (tumlUri.indexOf(tumlModelName, tumlUri.length - tumlModelName.length) !== -1) {
                tumlUriAdjusted = tumlUriAdjusted + "/umlgRoot";
            }

            //Change the browsers url
            if (pushUrl === undefined || pushUrl) {
                pushUrlToBrowser(tumlUriAdjusted);
            }
            var contextVertexId = retrieveVertexId(tumlUriAdjusted);
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    retrieveMetaDataIfNotInCache(tumlUriAdjusted, contextVertexId, result, continueRefresh);
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    alert('error getting ' + tumlUriAdjusted + '\n textStatus: ' + textStatus + '\n errorThrown: ' + errorThrown)
                }
            });
        }
