    ...
    function TumlTabQueryManager(parentTumlTabViewManager, instanceQueryUri, classQueryUri, rootQueryUri, queryId) {
        ...
        this.afterExecuteQuery = function (data) {
            if (Array.isArray(data)) {
                ...
                var outerDivForResults = $('#' + this.queryTabDivName + '_' + 'OclResult');
                outerDivForResults.children().remove();
                var sqlGridResult = $('<div />', {id: 'sqlQueryResultId', style: 'width:auto;height:500px;'});
                sqlGridResult.appendTo(outerDivForResults);

                var columns = [data.meta.length];
                for (i = 0; i < data.meta.length; i++) {
                    var meta = data.meta[i];
                    columns[i] = {id: meta.name, name: meta.name, field: meta.name};
                }
                ...
                var data = data.data;
                var grid = new Slick.Grid('#sqlQueryResultId', data, columns, options);
