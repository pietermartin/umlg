...
//            var derived = 'derived: ' + propertyNavigatingTo.derived;
    ...
    TumlTabContainerManager.prototype.addQueryButtons = function (query) {
        ...
        //Create the dropdown query type options
        var oclLink = $('<li><a href="#">OCL</a></li>').appendTo(ul);
...
//        this.executeButton = this.queryExecuteButtonFormGroupDiv.find('#executeButton');
        this.executeButton.text('Execute ' + query.type.toUpperCase());
        this.executeButton.val(query.type.toUpperCase());
        this.executeButton.button().unbind("click");
        this.executeButton.button().click(
            function () {
                self.executeQuery();
            }
        );
    ...
    TumlTabContainerManager.prototype.addOrRemoveSelectButton = function () {
        if (this.propertyNavigatingTo !== undefined && this.propertyNavigatingTo !== null &&
            !this.propertyNavigatingTo.associationClassOne && !this.propertyNavigatingTo.derived &&
            !this.propertyNavigatingTo.composite && (this.propertyNavigatingTo.upper === -1 || this.propertyNavigatingTo.upper > 1) && !this.oneManyOrQuery.forLookup) {
